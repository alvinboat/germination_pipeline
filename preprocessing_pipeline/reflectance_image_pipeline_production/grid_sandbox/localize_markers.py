...
reflectance cube -- WITHOUT relying on cv2.aruco's own bit-decode, which
fails on this camera's push-broom-blurred captures (confirmed: candidate
squares are found fine, decode against every dictionary/param combo is not;
see grid_sandbox/detect_markers_check.py and check_pre_resample.py).

Instead: take every quad cv2.aruco's detector considers a marker candidate
(both successfully-decoded AND rejected), filter to ones sized/shaped like
an actual marker, then identify each by normalized cross-correlation against
the two known reference patterns (inverted polarity -- these markers are
3D-printed inverted, confirmed empirically: correlation against the normal
render is negative across the board, against the inverted render it's a
clean 0.5-0.65). Best-scoring candidate per id wins.

x = width/spatial axis, y = scan-line axis -- same convention as
box_line_overlay.py (gray8 = intensity_preview(cube).T).

Usage:
    python3 localize_markers.py [<corrected_cube.npy>]
        -> grid_sandbox/<name>_markers.json, <name>_markers_overlay.png
"""
import argparse
import json
import sys
...
DEFAULT_CUBE = Path(__file__).resolve().parent.parent / "corrected_file" / "716_2.npy"
...
# Empirical marker size in this cube's pixel grid (~80x77px on 716_2's 640-wide
# frame) -- candidates must be within this band to be considered, or every
# grid-cell corner/kernel-blob quad in the frame becomes a "candidate" too.
...
def load_gray8(cube_path):
    ...
    return gray.T                  # (n_lines, width): rows=y=scanline, cols=x=width
...
def draw_overlay(gray8, markers):
    vis = cv2.cvtColor(gray8, cv2.COLOR_GRAY2BGR)
    colors = {0: (0, 0, 255), 1: (0, 255, 0)}
    for mid, m in markers.items():
        pts = np.array(m["corners"], dtype=np.int32)
        cv2.polylines(vis, [pts], True, colors.get(mid, (255, 255, 0)), 2)
        cx, cy = map(int, m["center"])
        label = f"id{mid} s={m['score']:.2f}"
        cv2.putText(vis, label, (cx - 30, cy - 45), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(vis, label, (cx - 30, cy - 45), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    colors.get(mid, (255, 255, 0)), 1, cv2.LINE_AA)
    return vis


def main():
    p = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("cube", nargs="?", default=str(DEFAULT_CUBE))
    args = p.parse_args()

    gray8 = load_gray8(args.cube)
    markers = localize_markers(gray8)

    name = Path(args.cube).stem
    for mid in MARKER_IDS:
        if mid not in markers:
            print(f"warning: marker id {mid} not found above confidence threshold")
            continue
        m = markers[mid]
        print(f"id={mid}: center=({m['center'][0]:.1f}, {m['center'][1]:.1f}) "
              f"score={m['score']:.3f} margin={m['margin']:.3f}")

    out_json = OUT_DIR / f"{name}_markers.json"
    with open(out_json, "w") as f:
        json.dump(markers, f, indent=2)
    print(f"wrote {out_json}")

    out_png = OUT_DIR / f"{name}_markers_overlay.png"
    cv2.imwrite(str(out_png), draw_overlay(gray8, markers))
